...
All detector methods are present in the shipped `artoolkit5.wasm` (`@ar-js-org/artoolkit5-wasm@0.1.3`), confirmed by symbol search:
...
### Constants available (`@ar-js-org/artoolkit5-constants@0.2.0`)

Reaching us transitively via `artoolkit5-wasm@0.1.3`. 55 constants total, including all five detection modes (both combined) and all eleven matrix code types. **The claims in #8 and #9 that these are missing are stale** — they describe `0.1.0`, before the fixes released in `0.2.0`.

### Two gaps found during design

| Gap | Evidence | Handling |
|---|---|---|
| `arLabelingMode` constants not generated | `artoolkit5-constants` CHANGELOG, "Still missing"; extractor has zero references | `labelingMode` omitted from #8. Filed as [artoolkit5-constants#6](https://github.com/AR-js-org/artoolkit5-constants/issues/6) |
| `AR_LABELING_THRESH_MODE_AUTO_ADAPTIVE` silently degrades to `MANUAL` | `AR_DISABLE_THRESH_MODE_AUTO_ADAPTIVE=1` in `config.h` compiles its `case` out; falls through to `default:` | `'auto-adaptive'` omitted from the union |
...
1. `@ar-js-org/artoolkit5-constants` becomes a **direct** dependency at `^0.2.0`. Zero extra install cost — `artoolkit5-wasm@0.1.3` already requires the same range, so npm dedupes to one copy.
...
| R4 | `labelingMode` stays unavailable | Tracked upstream in artoolkit5-constants#6. Purely additive when it lands. |
...
- **artoolkit5-constants#6** — generate `arLabelingMode` and `arMarkerExtractionMode` constants; unblocks `labelingMode`.
- **artoolkit5-constants#2** — appears resolved by `0.2.0`; verify and close.
- **Correct #8 and #9** — replace the stale `constants@0.1.0` rationale, fix #9's `#06` reference to #8, expand the matrix type list from six to eleven, and replace #9's combined-mode risk with the `getMarkerInfo` finding.
- **`docs/issues/*.md`** — add a header to each draft mapping it to its filed issue number.
- **Upstream (optional)** — bind `idPatt`/`idMatrix`/`cfPatt`/`cfMatrix` in `artoolkit5-wasm` so pattern-vs-barcode could be read from the engine rather than derived. Not needed given decision 3.
