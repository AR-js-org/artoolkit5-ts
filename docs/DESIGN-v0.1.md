...
8. **Teardown:** `disposeARToolKitState` calls `core.teardown()` and then Embind's generated `core.delete()`. **Verified** — both exist on a live instance. An earlier claim here that no destructor symbol existed was wrong: it came from a symbol-extraction filter that only matched names beginning with `get|set|add|load|detect|pass|setup|is|has|marker`, which silently dropped `teardown`.
...
Calls `core.teardown()` to release the ARToolKit handles, then `core.delete()` to free the C++ instance, and sets `disposed = true`. Both steps are needed: `delete()` alone leaks the handles the instance owns. Every public function guards on `state.disposed` and throws `ARToolKitError` if called afterwards, turning a WASM-level crash into a clear TypeScript error.
...
| ~~R6~~ | ~~`core.delete()` may not exist~~ | — | **Resolved.** `teardown()` is bound in `ARToolKitCore_bindings.cpp` and `delete()` / `isDeleted()` are generated by Embind; both confirmed present on a live instance. |
